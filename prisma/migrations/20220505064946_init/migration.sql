...
CREATE TABLE `ReportLog` (
    ...
    UNIQUE INDEX `ReportLog_reportId_key`(`reportId`),
...
CREATE TABLE `OXUnitRange` (
    ...
    UNIQUE INDEX `OXUnitRange_roomId_key`(`roomId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `PlanDay` ADD CONSTRAINT `PlanDay_userId_fkey` FOREIGN KEY (`userId`) REFERENCES `User`(`userId`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `PlanDay` ADD CONSTRAINT `PlanDay_planId_fkey` FOREIGN KEY (`planId`) REFERENCES `Plan`(`planId`) ON DELETE CASCADE ON UPDATE CASCADE;
